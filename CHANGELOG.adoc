...
Build::
  ...
  * Make asciidoctorj usable with the Java 9+ module system. To avoid the 'split-jar' problem, some classes
  have moved from package `org.asciidoctor` to `org.asciidoctor.api`. This is true also the
  packages `org.asciidoctor.extension` (to `org.asciidoctor.api.extension`)  and `org.asciidoctor.converter`
  (to `org.asciidoctor.api.converter`).
...
Documentation::
  ...
  * Upgrade to Asciidoctor EPUB3 1.5.0-alpha.14
