...
The builder `OptionsBuilder` allows for simple configuration of that instance that can be passed to the respective methods of the `Asciidoctor` interface.
...
OptionsBuilder optionsBuilder =
            OptionsBuilder.options().inPlace(true);
----
...
----
+
. Use `AttributesBuilder` methods for a fluent API.
+
[source,java]
