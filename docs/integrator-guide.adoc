...
|Insert content into the `header` element (html) or `docinfo` element (docbook)
|===
...
It was noted at the start of this section that the target (the x in `\menu:x[]`) is optional. 
...
// From an issue somewhere but I cant find it

[NOTE]
--
If you have installed AsciidoctorJ as recommended, asciidoctorj will be on the path, and you can use:

 asciidoctorj -cp=lib/myextension.jar test.adoc

If you have downloaded the distribution jars only, use a command like:

 java -cp lib/asciidoctor-api-2.0.0.jar;lib/asciidoctor-core-2.0.0.jar;lib/jcommander-1.35.jar;lib/myextension.jar  org.asciidoctor.jruby.cli.AsciidoctorInvoker  test.adoc
