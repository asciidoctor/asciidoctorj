...
To get an own converter running in {uri-repo}[AsciidoctorJ] these steps are required:

- Implement the converter as a subclass of `org.asciidoctor.converter.AbstractConverter`.
- Register the converter with a well defined backend name at the `ConverterRegistry`.
- Pass the backend name to the `Asciidoctor` instance when rendering a source file.
...
----
Asciidoctor asciidoctor = Asciidoctor.Factory.create();
asciidoctor.javaConverterRegistry().register(TextConverter.class, "text"); // <1>
----
<1> Registers the converter class `TextConverter` with the backend name `text`.
...
public class TextConverterRegistry implements ConverterRegistry{
    ...
    public void register(Asciidoctor asciidoctor) {
        asciidoctor.javaConverterRegistry().register(TextConverter.class, "textt");
    }
}
----
...
To render a document with this converter the backend name `text` has to be passed via the options:

[source,java]
----
File adocFile = ...
asciidoctor.renderFile(adocFile, OptionsBuilder.options().backend("text"));
----
