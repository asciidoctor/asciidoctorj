...
Either: use the jboss-cli and give it the following commands:
...
module add \
 --name=org.asciidoctor \ --resources=lib/asciidoctorj-1.5.2.jar,lib/jcommander-1.35.jar,lib/jruby-complete-1.7.16.1.jar \
 --resource-delimiter=, \
...
. Make sure the version numbers of the jar files agree with what's in your application. Restart WildFly for the new module to take effect.
