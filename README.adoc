...
image:https://img.shields.io/badge/javadoc.io-{artifact-version}-blue.svg[Javadoc, link=http://www.javadoc.io/doc/org.asciidoctor/asciidoctorj/{artifact-version}]
endif::[]
...
CAUTION: The artifactId changed from *asciidoctor-java-integration* to *asciidoctorj* starting with version 1.5.0.
...
NOTE: If you plan to write extensions for Asciidoctor you should seriously consider using the latest version of the 1.6.0 family.
      While the current version is an alpha version it contains the same solid version of Asciidoctor.
      The Extension API has changed a bit, in an incompatible way though, but is much more solid when it comes to extensions.
      To not break compatibility with existing users of the Extension API it is not possible to bring these stabilization efforts to the current 1.5 family.
...
public class TerminalCommandTreeprocessor extends Treeprocessor { // <1>
  ...
  public Document process(Document document) {
    ...
    final List<AbstractBlock> blocks = this.document.getBlocks(); // <2>
...
JavaEmbedUtils.initialize(Arrays.asList("META-INF/jruby.home/lib/ruby/2.0", "gems/asciidoctor-{artifact-version}/lib"), config); <2><3><4>
